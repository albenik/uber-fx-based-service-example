...
const hashKey = "feature_toggles"

// toggleDTO is the JSON value stored per toggle in the Redis hash.
type toggleDTO struct {
	Enabled     bool   `json:"enabled"`
	Description string `json:"description"`
}

// Provider implements ports.FeatureToggleProvider with Redis as the storage
// backend and project-local evaluation logic. Toggles are kept in a single
// Redis hash ("feature_toggles") where each field is the toggle name and the
// value is a JSON object carrying the enabled flag and description.
//
...
func NewProvider(client *redis.Client, logger *zap.Logger) *Provider {
	return &Provider{client: client, logger: logger}
}

func (p *Provider) IsEnabled(ctx context.Context, name string) (bool, error) {
	raw, err := p.client.HGet(ctx, hashKey, name).Result()
	if err == redis.Nil {
		return false, nil
	}
	if err != nil {
...
func (p *Provider) GetToggle(ctx context.Context, name string) (*domain.FeatureToggle, error) {
	raw, err := p.client.HGet(ctx, hashKey, name).Result()
	if err == redis.Nil {
...
func (p *Provider) ListToggles(ctx context.Context) ([]*domain.FeatureToggle, error) {
	all, err := p.client.HGetAll(ctx, hashKey).Result()
	if err != nil {
...
var _ ports.FeatureToggleProvider = (*Provider)(nil)
