...
- **gRPC adapter** (`internal/adapters/out/grpc/featuretoggle/`): Delegates all logic to a remote feature-toggle service via gRPC (`proto/featuretoggle/v1/`).
...
- **Backend selection** (`internal/core/services/featuretoggle/`): Service that routes to the correct protocol adapter based on `FEATURE_TOGGLE_BACKEND` config.
...
**Services** (`internal/core/services/`) — Business logic: `legalentity/`, `fleet/`, `vehicle/`, `driver/`, `contract/`, `assignment/`, `featuretoggle/` (backend selection service).
...
**Output Adapters** (`internal/adapters/out/`) — Organized by protocol, not by domain feature. Each protocol directory contains sub-packages when multiple independent services share the same protocol. `postgres/`: PostgreSQL repository implementations with master/replica connection pools (sqlx over pgx/v5 stdlib), goose migrations (embedded), plain SQL, DTO structs with `db` tags for row mapping. `grpc/`: gRPC clients with sub-packages per service — `driverlicense/` for driver license validation and `featuretoggle/` for feature toggle provider (separate hosts and configs). `redis/`: Redis-backed implementations — feature toggle provider (`FeatureToggleProvider`). Backend selection for feature toggles lives in the services layer (`services/featuretoggle/`) and delegates to the correct protocol adapter.
